...
const newContent = `            <!-- Remodeled Full-Width Profile -->
            ...
            <div style="padding: 1.5rem; background: var(--bg-body); border-radius: 12px; margin-bottom: 2rem;">
                ...
                {#if entry.notes}
                    <div class="service-notes" style="font-size: 1.05rem; line-height: 1.6; margin-bottom: 1.5rem; white-space: pre-wrap; color: var(--text-faint);">
                        {entry.notes}
                    </div>
                {/if}
                ...
                {#if entry.tags}
                    <div class="service-tags">
                        {#each entry.tags.split(" ") as t}
                            <span class="service-tag" style="padding: 0.4rem 0.8rem; font-size: 0.95rem;">{t.startsWith("#") ? t : "#" + t}</span>
                        {/each}
                    </div>
                {/if}
            </div>`;

const startRegex = /<article\s+class="card service"[\s\S]*?\{entry\.category\}\s*<\/div>/;
const endRegex = /<\/article>/;

// We will just do string replacement between <article> and </article>
