...
"""
    ...
    # Currently .entry-tag is: background: var(--entry-category-bg);
    # And in dark mode, we just set --entry-category-bg: var(--overlay-light); 
    # Or in dark mode, --entry-category-bg: #222222; which works too. So we can just delete lines 405-407 `[data-theme="dark"] .entry-tag { ... }`
    content = re.sub(r'\[data-theme="dark"\]\s*\.entry-tag\s*\{[^}]+\}', '', content)
