<script>
    ...
    onMount(async () => {
        try {
            page = await apiFetch("/api/pages?slug=iranyelvek");
        } catch {
            error = true;
        } finally {
            loading = false;
...
<svelte:head>
    <title>{page?.title || "Irányelvek"} – Lámsza</title>
</svelte:head>

<Breadcrumbs label="Irányelvek" parentLabel="" parentUrl="" />

<section class="page-section">
    <h1 class="page-title">{page?.title || "Irányelvek"}</h1>
